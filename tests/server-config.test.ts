...
describe("Azure browser read configuration", () => {
  it("accepts an HTTPS read-only SAS scoped to the configured path", () => {
    expect(
      validateAzureReadBaseUrl(
        "https://account.blob.core.windows.net/thimbledb/demo?sp=r&sig=test",
        "thimbledb",
        "demo",
      ),
  ...
  it("rejects a SAS with write or delete permissions", () => {
    expect(() =>
      validateAzureReadBaseUrl(
        "https://account.blob.core.windows.net/thimbledb/demo?sp=rwd&sig=test",
        "thimbledb",
        "demo",
      ),
  ...
  it("rejects a URL that omits the container or prefix", () => {
    expect(() =>
      validateAzureReadBaseUrl(
        "https://account.blob.core.windows.net/content-trie?sp=r&sig=test",
        "thimbledb",
        "demo",
      ),
